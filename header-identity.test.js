...
test("chip prefers the selected character name and prints realm + version as text", () => {
  ...
  assert.equal(model.character, "Nightbloom");
  assert.equal(HID.displayName(model), "Nightbloom");
  assert.equal(HID.selectedSummary(model), "Selected: Proudmoore US · Retail");
  const html = HID.chipHTML(model);
  assert.match(html, /Nightbloom/);
