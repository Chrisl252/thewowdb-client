...
(function (root, factory) {
  ...
  function identityModel(auth, storage) {
    ...
    var ch = auth && auth.active_character;
    var character = signedIn && ch && ch.name ? String(ch.name) : "";
    var model = {
      signedIn: signedIn,
      account: account,
      character: character,
  ...
  function displayName(model) {
    return (model && (model.character || model.account)) || "";
  }

  function selectedSummary(model) {
    if (!model || !model.signedIn) return "";
  ...
  function chipHTML(model) {
    ...
    var name = displayName(model);
    var realm = model.realm || "Choose realm";
    var version = model.version || "Retail";
    return (
      '<span class="wgf-header-identity__account">' + esc(name) + "</span>" +
  ...
  function chipLabel(model) {
    ...
    return displayName(model) + " — " + (model.realm || "no realm") + " — " + (model.version || "Retail") +
